...
| <a name="input_gateway_enabled"></a> [gateway\_enabled](#input\_gateway\_enabled) | Front the cluster with a single Gateway API Gateway (modules/gateway,<br>Envoy Gateway) instead of ingress-nginx + a separate mc-router<br>LoadBalancer. When true: HTTPRoutes are created for the jhub / triage /<br>example hosts and `extra_http_routes`, a TCPRoute fronts mc-router<br>(which drops to ClusterIP), cert-manager's gateway-shim is enabled, and<br>external-dns switches to the gateway-httproute / gateway-tcproute<br>sources. Pair with `ingress_nginx_enabled = false` once DNS has moved to<br>finish on exactly one NodeBalancer. | `bool` | `false` | no |
...
| <a name="input_ingress_nginx_enabled"></a> [ingress\_nginx\_enabled](#input\_ingress\_nginx\_enabled) | Deploy ingress-nginx (+ hairpin-proxy). ingress-nginx is unmaintained since March 2026; set false after `gateway_enabled = true` has taken over DNS. Uninstall reaps its NodeBalancer (`preserve=false`). | `bool` | `true` | no |
