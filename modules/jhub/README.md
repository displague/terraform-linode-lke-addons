...
| <a name="input_hub_db_claim"></a> [hub\_db\_claim](#input\_hub\_db\_claim) | Name of the terraform-managed PVC that stores the hub sqlite database. Default matches the historical chart-generated name for zero-downtime migration from `hub.db.type = sqlite-pvc`. | `string` | `"hub-db-dir"` | no |
...
| <a name="input_hub_db_storage_size"></a> [hub\_db\_storage\_size](#input\_hub\_db\_storage\_size) | Size of the hub sqlite PVC. 10Gi is the historical chart default via linode-block-storage. | `string` | `"10Gi"` | no |
| <a name="input_hub_db_volume"></a> [hub\_db\_volume](#input\_hub\_db\_volume) | Optional pre-existing PersistentVolume name to bind the hub sqlite PVC to. Set this when adopting the module for an existing chart-managed PVC (`Retain` policy preserves the PV across the migration). Leave "" to let the storage class dynamically provision a fresh PV. | `string` | `""` | no |
